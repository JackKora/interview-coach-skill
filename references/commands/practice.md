...
- **Round format from Interview Loops**: Match the exact Loop ID or company + role + upcoming round before selecting drill types from `prep` Format Discovery or Interview Loops; disambiguate multiple matches. A presentation round → prioritize structure and timing drills. A panel interview → prioritize panel drill. A system design round → prioritize `practice technical`. The drill should simulate the conditions they'll actually face.
...
2. **Check Format Discovery data.** Match the exact Loop ID or company + role + round before using a saved format from Interview Loops; disambiguate multiple matches. Use Profile format only when it is explicitly general rather than loop-specific. Then reference it: "You told me your system design round is a collaborative verbal walkthrough. I'll tailor the drills to that format."
...
2. **Tailor questions to target roles.** Pull from `coaching_state/profile.md` and `coaching_state/interviews.md` — use the candidate's target companies, JDs, predicted questions from `prep`, and known weak competencies. Don't use generic questions if role-specific data exists.
