...
1. Ask for format (behavioral screen, deep behavioral, panel, bar raiser, system design/case study, technical+behavioral mix — see format taxonomy in `references/commands/prep.md`). **For system design/case study and technical+behavioral mix**: Match the exact Loop ID or company + role + round before reading saved format data from `prep`; disambiguate multiple matches. If no matching format data exists, run the Format Discovery Protocol (see `references/commands/prep.md`) and save it under that exact loop and round. See format-specific simulation UX sections below.
2. Ask for company/role context (or use existing prep data).
3. **Calibrate difficulty to the candidate's progression stage.** Check Drill Progression in `coaching_state/profile.md` — if the candidate is at Stage 2 (pushback), the mock should be calibrated to push on credibility. If they're at Stage 6+ (panel/stress), they can handle maximum intensity. Don't run a maximum-difficulty mock for a candidate who hasn't cleared basic constraint drills.
4. **Calibrate tone to the target company and role.** A mock for a FAANG final round should feel very different from a Series A startup first call:
   ...
   - If prep data exists for the exact loop, use the culture read and format analysis to shape the mock's feel.
...
4. Include at least one question targeting a known story gap (from storybank gap analysis or `coaching_state/storybank.md`) to test gap-handling under realistic conditions. When the candidate encounters this gap question, evaluate their response against the Gap-Handling Module patterns in `references/cross-cutting.md`. Note which pattern they used (or didn't) in the per-question debrief.
5. **Pull from saved concerns data.** If `concerns` was previously run for the exact Loop ID or company + role (check `coaching_state/interviews.md` or the general Active Coaching Strategy), include at least one question that targets the top-ranked concern. Disambiguate multiple matching loops. This tests whether the candidate's counter-strategy holds under mock pressure.
...
**Before starting, check the exact Loop ID or company + role + round for saved format data from `prep`.** If multiple records match, disambiguate. If no matching format data exists, run the Format Discovery Protocol (see format taxonomy in `references/commands/prep.md`) and save it under that exact loop and round. If the candidate has described their specific format, simulate THAT. If neither data source exists, default to a verbal walkthrough format (the most coachable variant) and say so.
